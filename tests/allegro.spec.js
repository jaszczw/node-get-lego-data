...
const getSetAuctions = require('../lib/allegro/getSetAuctions');
const jexpect = expect;

describe('allegro', ()=> {
  var allegroClientMock,
      soapMock,
      result = { itemsCount: 1, itemsList: null };
  ...
  describe('getSetAuctions', () => {
    it('makes proper request to a soap', function () {
      return getSetAuctions(soapMock)('setId')
          .then(function () {
    ...
    it('makes returns result from soap call', function () {
      return getSetAuctions(soapMock)('setId')
          .then(function (actualResult) {
            ...
            expect(actualResult).to.deep.equal({ itemsCount: 1, itemsList: [] });
          });
    });
  });
});
